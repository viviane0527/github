...
import data.persistence.hubs.ManagerHubFacade;
...
@Stateless
@Path("managers/")
public class ManagerFacadeREST implements Serializable {

	@Inject
    private ManagerHubFacade managerHubFacade;
    ...
    public Manager create(Manager entity) {
        return managerHubFacade.create(entity);
    }
    ...
    @Consumes({"application/xml", "application/json"})
    @Produces({"application/xml", "application/json"})
    public Manager edit(@PathParam("id") Long id, Manager entity) {
        return managerHubFacade.edit(entity);
    }

    @DELETE
    @Path("{id : \\d+}")
    public void remove(@PathParam("id") Long id) {
        managerHubFacade.remove(managerHubFacade.find(id));
    }

    @GET
    @Path("{id : \\d+}")
    @Produces({"application/xml", "application/json"})
    public Manager find(@PathParam("id") Long id) {
        return managerHubFacade.find(id);
    }
    ...
    public Response findRange(
        ...
        List<Manager> managers = managerHubFacade.searchRange(range,
		...
		return Response.ok(
			new GenericEntity<List<Manager>>(new ArrayList(managers)) {}
